...
router.post("/updateSettings", loginRequired, async (req: RequestWithUser, res: Response) => {
    if(userIsDefined(req)){
        const userSettings: IfUserSettings = req.body;
        const response = await userSetgns.updateSettings(userSettings, req.user.id);
        res.status(response.statusCode).json(response.result);
    }
});

router.get("/getSettings", loginRequired, async (req: RequestWithUser, res: Response) => {
    if (userIsDefined(req)) {
