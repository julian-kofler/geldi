...
export class UserSetgns{
    ...
    async updateSettings(userSettings: IfUserSettings, userId: number): Promise<{statusCode: number, result: any}> {
        const emailCheck = await this.inputVal.isValidEmail(userSettings.email);
        const passwordCheck = await this.inputVal.isValidPassword(userSettings.password);
        ...
        try {
            const sql = "UPDATE users SET nickname = ?, email = ?, password = ? WHERE id = ?";
            await this.db.query(sql, [userSettings.nickname, userSettings.email, userSettings.password, userId]);
    ...
    async getSettings(userId: number): Promise<{statusCode: number, result: any}> {
        try {
