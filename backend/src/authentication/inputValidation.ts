...
export class InputValidation {
  private commonPasswordsURL: string =
    "https://lucidar.me/en/security/files/100000-most-common-passwords.json";
  private commonPasswordsPath: string = "";

  constructor() {
    this.scheduleAutomaticTasks();
    this.commonPasswordsPath = path.join(
      fileURLToPath(import.meta.url),
      "../../../data/" + this.commonPasswordsURL.split("/").pop()!
    );
  ...
  private async isInCustomList(password: string): Promise<boolean> {
    const commonPasswords = JSON.parse(
      fs.readFileSync(this.commonPasswordsPath, "utf8")
    );
    return commonPasswords.includes(password);
  }

  private async isWeakPassword(
    password: string
