...
export class Auth {
  ...
  private async getPasswordReset(
    hashedToken: string
  ): Promise<PasswordReset | null> {
    try {
      const sql =
        "SELECT token, userId, expirationTime FROM password_resets WHERE token = ?";
  ...
  private createJWT(userId: number): string {
    return jwt.sign({ userId: userId }, process.env.JWT_SECRET!, {
      expiresIn: this.jwtValidityTime,
    });
  }
  ...
  private async createRefreshToken(userId: number): Promise<string> {
    try {
      ...
      this.refreshTokenExpirationTime.setMinutes(
        this.refreshTokenExpirationTime.getMinutes() +
      ...
      const sqlInsert =
        "INSERT INTO validRefreshTokens (token, userId, expirationTime) VALUES (?, ?, ?)";
      const values = [
        hashedRefreshToken,
        userId,
  ...
  private async getValidRefreshToken(
    ...
    try {
      const sql =
        "SELECT token, userId, expirationTime FROM validRefreshTokens WHERE token = ?";
      const [rows] = await this.db.query<ValidRefreshToken[]>(sql, [
        hashedToken,
      ]);
      if (rows.length > 0) {
        return rows[0];
  ...
  public async refreshJWT(
    token: string
  ): Promise<{ statusCode: number; result: jwtRefreshResponse }> {
    try {
      if (!this.isRefreshTokenValid(token)) {
        return {
          statusCode: 401,
          result: { message: "Invalid refresh token", jwt: "" },
        };
      }
      ...
      return {
        statusCode: 200,
        result: { message: "Token refreshed successfully", jwt: newToken },
      };
    } catch (error) {
      logger.error((error as Error).message);
      return {
        statusCode: 500,
        result: { message: "Error refreshing token", jwt: "" },
      };
    }
  }
  ...
  public async newRefreshToken(
    token: string
  ): Promise<{ statusCode: number; result: refreshTokenResponse }> {
    try {
      if (!this.isRefreshTokenValid(token)) {
        return {
          statusCode: 401,
          result: { message: "Invalid refresh token", refreshToken: "" },
        };
      }
      ...
      return {
        statusCode: 500,
        result: { message: "Error refreshing refresh token", refreshToken: "" },
      };
    }
  }
  ...
  public async signUp(
    ...
    password: string,
    nickname: string
  ): Promise<{ statusCode: number; result: SignUpResponse }> {
    ...
    if (!emailCheck.valid) {
      return {
        statusCode: 400,
        result: { message: emailCheck.message, jwt: "", refreshToken: "" },
      };
    }
    if (!passwordCheck.valid) {
      return {
        statusCode: 400,
        result: { message: passwordCheck.message, jwt: "", refreshToken: "" },
      };
    }
  ...
  public async login(
    ...
    try {
      ...
      return {
        statusCode: 500,
        result: { message: "Error logging in user", jwt: "", refreshToken: "" },
      };
    }
  }
    ...
    try {
      ...
      if (!user) {
        return {
          statusCode: 200,
          message:
            "If the account exists, the password reset email has been sent successfully",
        };
      }
      ...
      this.resetTokenExpirationTime.setMinutes(
        this.resetTokenExpirationTime.getMinutes() + this.resetTokenValidityTime
      );

      const sql =
        "INSERT INTO password_resets (token, userId, expirationTime) VALUES (?, ?, ?)";
      ...
      const mailOptions = {
        ...
        text:
          `Sie erhalten diese E-Mail, weil Sie (oder jemand anderes) eine Zurücksetzung des Passworts für Ihr Konto angefordert haben.\n\n` +
  ...
  public async passwordReset(
    ...
    try {
      ...
      const resetInfo: PasswordReset | null = await this.getPasswordReset(
        hashedRequestToken
      );
      ...
      const user: User | null = await this.userRepo.getUserByID(
        resetInfo.userId
      );
